...
export default function ChatDrawer({
  ...
  return (
    <div className="fixed bottom-0 right-4 sm:right-10 w-[340px] sm:w-[380px] bg-white rounded-t-3xl shadow-2xl border-t border-x border-gray-100 z-50 overflow-hidden flex flex-col animate-slide-up select-none">
      ...
      {/* Header */}
      <div className="bg-[#5F3041] text-white p-4 flex justify-between items-center">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-full bg-white/25 flex items-center justify-center font-bold text-xs relative flex-shrink-0">
            {activeChat.initials}
            {activeChat.online && (
              <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-emerald-500 rounded-full border border-[#5F3041]" />
            )}
          </div>
          <div>
        ...
        <button 
          ...
          className="text-white/70 hover:text-white cursor-pointer border-none bg-transparent flex items-center"
        >
            ...
            <div key={idx} className={`flex flex-col max-w-[80%] ${isUser ? 'ml-auto items-end' : 'mr-auto items-start'}`}>
              <div
                className={`p-3 rounded-2xl text-xs leading-relaxed ${isUser
                  ? 'bg-[#5F3041] text-white rounded-br-none'
      ...
      <form onSubmit={sendChatMessage} className="p-3 border-t border-gray-100 flex gap-2 bg-white">
        <input
          ...
          className="flex-1 border border-gray-100 bg-gray-50 rounded-full px-4 py-2 text-xs focus:outline-none focus:bg-white focus:border-gray-200 focus:ring-1 focus:ring-[#5F3041]/20"
          style={{ fontFamily: 'var(--font-montserrat)' }}
        />
        <button
          type="submit"
          className="bg-[#5F3041] text-white p-2 rounded-full hover:bg-[#4A2231] transition-colors cursor-pointer flex items-center justify-center w-8 h-8 flex-shrink-0 shadow border-none"
        >
